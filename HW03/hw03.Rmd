...
Green_Rescale <- green_data %>%
  mutate(across(all_of(reversed), ~ recode(.x, "1" = 0, "2" = 25, "4" = 75, "5" = 100), .names = "{.col}_pomp")) %>%
  rowwise() %>%
  mutate(
    green_pomp = mean(c_across(green1 : green5), na.rm = TRUE),
...
ggplot(Green_Rescale) +
  aes(x = green_pomp) +
  geom_histogram(fill = "green", fill = "purple") +
  theme_bw() +
  ggtitle("Green Data")
