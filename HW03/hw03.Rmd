...
```{r}
Green_R <- Green_New %>%
  mutate(across(all_of(green_data), 
                ~ recode(.x, 
                         "5" = "1",
                         "4" = "2",
                         "3" = "3",
...
Green_Calculations <- Green_R %>%
  ...
  na.omit() %>%
  select(id, student, total_green, total_comp, total_intel, total_open)
```
...
```{r}
Green <- Green_Calculations %>%
  mutate(
    green_pomp = rescale (total_green, to = C(0, 100), from = C(1,5)),
    comp_pomp = rescale (total_comp, to = C(0, 100), from = C(1,5)), 
    intel_pomp = rescale (total_intel, to = C(0, 100), from = C(1,5)),
    open_pomp = rescale (total_open, to = C(0, 100), from = C(1,5))) %>%
